...
PangoLayout* makeLayout(cairo_t* cr, const std::string& font, const std::string& text, double px,
    ...
    if (maxWidthPx > 0) {
        // Constrain + ellipsize so the text never renders past its box.
        pango_layout_set_width(layout, static_cast<int>(maxWidthPx * PANGO_SCALE));
...
std::string cacheKey(const std::string& text, double px, float r, float g, float b,
                     double maxWidthPx) {
    // px and maxWidth are quantised to ints; colours to bytes — plenty for de-duping.
    char buf[80];
    std::snprintf(buf, sizeof(buf), "|%d|%02x%02x%02x|%d", static_cast<int>(px + 0.5),
                  static_cast<int>(r * 255), static_cast<int>(g * 255), static_cast<int>(b * 255),
                  static_cast<int>(maxWidthPx + 0.5));
    return text + buf;
}
...
Vec2 TextRenderer::measure(const std::string& text, double px, double maxWidthPx) const {
    cairo_surface_t* tmp = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, 1, 1);
    cairo_t*         cr  = cairo_create(tmp);
    PangoLayout*     lay = makeLayout(cr, m_font, text, px, maxWidthPx);
...
SP<Render::ITexture> TextRenderer::textureFor(const std::string& text, double px, float r, float g, float b,
                                      Vec2& outSize, double maxWidthPx) {
    const std::string key = cacheKey(text, px, r, g, b, maxWidthPx);
    if (auto it = m_cache.find(key); it != m_cache.end()) {
        outSize = it->second.size;
        return it->second.tex;
    }

    // Measure (ellipsized to maxWidthPx if set), then rasterise into an ARGB32
    // surface of exactly that size.
    const Vec2 size = measure(text, px, maxWidthPx);
